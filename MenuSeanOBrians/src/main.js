...
// ── RENDER MENU IMAGE ──────────────────────────────────────────────────────
function renderMenuImage(section) {
  if (!section.menuImage) return ''
  return `
    <button class="menu-img-card" type="button" aria-label="${esc(t(section.menuImageAlt))}">
      <img src="${esc(section.menuImage)}" alt="${esc(t(section.menuImageAlt))}" loading="lazy">
      <span class="menu-img-overlay">
        <span class="menu-img-label">${currentLang === 'en' ? 'View full menu' : 'Ver carta completa'}</span>
      </span>
    </button>`
}
